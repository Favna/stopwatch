...
ava('restart', async (test): Promise<void> => {
	test.plan(2);
...
ava('reset', async (test): Promise<void> => {
	test.plan(2);
	const stopwatch = new Stopwatch();
	...
	test.true(stopwatch.duration === 0);
});

ava('start(stopped stopwatch)', async (test): Promise<void> => {
	test.plan(2);
...
ava('stopping(twice)', async (test): Promise<void> => {
	const stopwatch = new Stopwatch();
